...
def my_minimize(f, grad_f, x0, x_true, step, MAXITERATION, ABSOLUTE_STOP):
...
def deblur_immagini(original_img, b, K, maxit, _lambda=0, use_library=True):
    ...
    if(use_library):
        ...
        (x_last, ite, PSNR, MSE) = my_minimize(f_reg, df_reg, x0, original_img, step, MAXITERATION, ABSOLUTE_STOP)        
    ...
    if(color in [0, 1, 2]):
        ...
        for _lambda in array_lambda:
            ...
            if(use_library):
                ...
                plt.title(f'regular con libreria e lambda {_lambda} (PSNR: {PSNR_last: .2f} MSE: {MSE_last: .2f})', fontsize=20)          
            
            else:
                plt.title(f'regular con mio minimize e lambda{_lambda} (PSNR: {PSNR_last: .2f} MSE: {MSE_last: .2f})', fontsize=20)         
...
for img_name in ['Tychonoff.jpg', 'wallpaper.jpg']:
